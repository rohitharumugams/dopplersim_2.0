...
from doppler_sim.tf.experimental import (
...
from doppler_sim.specg.explorer import (
    ...
    SPECG_MAX_DURATION_S,
    SPECG_SR,
    SpecgAnalysisParams,
    build_all_spectrograms,
    specg_analysis_template_context,
    specg_build_reassignment_extras,
    specg_load_audio,
    specg_parse_analysis_params,
    specg_parse_fmax_from_form,
    specg_parse_fmax_hz,
)
...
def spectrograms():
    ...
    return render_template(
        ...
        fmax_presets=[500, 1000, 1250, 2000, 2500, 4000, 6000, 8000, 11025],
...
# ---------------------------------------------------------------------------
# Batch Generation (orchestration only — reuses Pass-By Sim synthesis backend)
# ---------------------------------------------------------------------------


def _batch_page_context(**extra: Any) -> dict[str, Any]:
    ...
    for name in catalog.sorted_vehicle_names():
        speeds = catalog.speeds_for(name)
        median = catalog.median_speed(name)
        length_m = known_length_m(name)
        vehicles.append(
            {
                "name": name,
                "display_name": vehicle_display_name(name),
                "length_m": length_m,
                "needs_length_input": length_m is None,
                "speeds": speeds,
                "speed_options": [
                    {
                        "mps": c.speed_mps,
                        "label": c.speed_label or f"{c.speed_mps:g} m/s",
                        "t_cpa1_s": c.t_cpa1_s,
                        "has_sidecar": c.sidecar_path is not None,
                    }
                    for c in catalog.vehicles[name]
                ],
                "median_speed": median,
                "clips": [c.stem for c in catalog.vehicles[name]],
            }
        )
    return {
        "active_tab": "batch",
        "batch_vehicles": vehicles,
        "batch_input_dir": input_dir_rel,
        "batch_input_default": DEFAULT_BATCH_INPUT_DIR,
        "batch_output_dir": output_dir_rel,
        "batch_output_default": DEFAULT_BATCH_OUTPUT_DIR,
...
def _parse_batch_input_dir(form) -> tuple[str | None, str | None]:
    from doppler_sim.batch.constants import DEFAULT_BATCH_INPUT_DIR, resolve_input_root

    raw = (form.get("input_dir") or DEFAULT_BATCH_INPUT_DIR).strip()
    try:
        resolve_input_root(BASE_DIR, raw)
    except ValueError as exc:
        return None, str(exc)
    return raw, None


def _parse_batch_output_dir(form) -> tuple[str | None, str | None]:
    from doppler_sim.batch.constants import DEFAULT_BATCH_NAME, DEFAULT_BATCH_OUTPUT_DIR, resolve_batch_output_root

    raw = (form.get("output_dir") or DEFAULT_BATCH_OUTPUT_DIR).strip()
    try:
        resolve_batch_output_root(BASE_DIR, raw)
    except ValueError as exc:
        return None, str(exc)
    return raw, None
...
def batch_generation():
    from doppler_sim.batch.constants import DEFAULT_BATCH_NAME, DEFAULT_BATCH_OUTPUT_DIR

    batch_name = request.args.get("batch_name", "")
    output_dir = (request.args.get("output_dir") or DEFAULT_BATCH_OUTPUT_DIR).strip()
    progress = None
    if batch_name:
        from doppler_sim.batch.runner import batch_progress

        raw_progress = batch_progress(BASE_DIR, batch_name, output_dir)
...
def batch_start():
    return _batch_start_or_resume(resume=False)


@app.route("/batch/resume", methods=["POST"])
def batch_resume():
    return _batch_start_or_resume(resume=True)


def _parse_batch_vehicle_lengths(form, selected_vehicles: list[str]) -> tuple[dict[str, float] | None, str | None]:
    from doppler_sim.batch.vehicle_metadata import known_length_m

    lengths: dict[str, float] = {}
    for name in selected_vehicles:
        known = known_length_m(name)
        if known is not None:
            lengths[name] = known
            continue
        raw = (form.get(f"length_{name}") or "").strip()
        if not raw:
            return None, f"Enter vehicle length (m) for {name} — no metadata entry found."
        try:
            value = float(raw)
        except (TypeError, ValueError):
            return None, f"Invalid length for {name}."
        if value <= 0:
            return None, f"Vehicle length must be positive for {name}."
        lengths[name] = value
    return lengths, None
...
def _batch_start_or_resume(*, resume: bool):
    ...
    if resume:
        if not plan_path.exists():
            return render_template(
                ...
                "Check the output folder and batch name, then try Resume again.",
                **_batch_page_context(
                    batch_name=batch_name,
                    batch_output_dir=output_dir,
        ...
        config = BatchPlan.load(plan_path).config
    else:
        ...
        if not catalog.sorted_vehicle_names():
            ...
            return render_template(
                ...
                "Add files named like CitroenC4Picasso_59.wav (speed in km/h) "
                "or sedan_10mps.wav (speed in m/s).",
                **_batch_page_context(
                    batch_name=batch_name,
                    batch_input_dir=input_dir,
                    batch_output_dir=output_dir,
            ...
            if clip.t_cpa1_s is None:
                return render_template(
                    ...
                    batch_error=(
                        ...
                        "'speed_kmh t_cpa1_s' (e.g. '54.0 5.65')."
                    ),
                    **_batch_page_context(
                        batch_name=batch_name,
                        batch_output_dir=output_dir,
        ...
        try:
            total_clips = max(1, int(request.form.get("total_clips", "10")))
        except (TypeError, ValueError):
            total_clips = 10
        ...
        config = BatchConfig(
            ...
            t_cpa1_s=parse_float("t_cpa1_s", 2.0),
            t_cpa2_min=parse_float("t_cpa2_min", 5.0),
            t_cpa2_max=parse_float("t_cpa2_max", 5.0),
    ...
    ok, message, initial_progress = start_batch_async(
        BASE_DIR,
        config,
        resume=resume,
...
@app.route("/batch/exists")
def batch_exists():
    from doppler_sim.batch.constants import DEFAULT_BATCH_OUTPUT_DIR
    from doppler_sim.batch.runner import batch_output_dir_exists
    ...
    return {
        "exists": batch_output_dir_exists(BASE_DIR, batch_name, output_dir),
        "batch_name": batch_name,
        "output_dir": output_dir,
    }
...
def batch_cancel():
    from doppler_sim.batch.runner import cancel_batch
    ...
    return render_template(
        ...
        **_batch_page_context(batch_name=batch_name),
    )
